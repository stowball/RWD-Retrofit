 ...
 */
;var rwdRetrofit=(function(){if(!document.querySelector||!document.getElementsByClassName||typeof(document.documentElement.clientWidth)=="undefined"){return}var r=document.querySelector('meta[name="viewport"]'),t=document.getElementsByClassName("rwdretro-desktop"),d=document.getElementsByClassName("rwdretro-mobile");if(!r||t.length===0||d.length===0){return}var q="content",k="media",b=r&&r.getAttribute(q),o="width=980",l=250,a="onorientationchange" in window,p=a?"orientationchange":"resize",j=t[0].getAttribute(k),s=d[0].getAttribute(k),f=d[0].getAttribute("data-mobile-breakpoint"),h,g={};if(cssua.ua.mobile&&f){h=f}else{h=j.replace(/.*?min-width:\s?(\d*).*/g,"$1")}j=j.replace(/(min-width:\s?)\d*/g,"$1"+h);s=s.replace(/(max-width:\s?)\d*/g,"$1"+(h-1));g.desktop=j,g.mobile=s;if(cssua.ua.mobile&&f){for(var n=0;n<t.length;n++){t[n].setAttribute(k,j)}for(var n=0;n<d.length;n++){d[n].setAttribute(k,s)}if(cssua.ua.ios){l=0}function e(){r.setAttribute(q,b);window.setTimeout(function(){if(document.documentElement.clientWidth>=h){r.setAttribute(q,o)}},l)}e();window.addEventListener(p,e,false)}return g})();